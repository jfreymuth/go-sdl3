...
//
// Returns the name of the rendering driver at the requested index, or an empty
...
//
// window_flags: the flags used to create the window (see [CreateWindow]).
//
// window: the window, or nil on error.
//
// renderer: the renderer, or nil on error.
//
// Returns the window and renderer or an error.
//
...
// don't need a specific renderer, specify "" and SDL will attempt to choose
...
//
// name: the name of the rendering driver to initialize, or "" to let
// SDL choose one.
//
// Returns a valid rendering context or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
//
//   - [PropRendererCreateNameString]: the name of the rendering driver
//     to use, if a specific one is desired
//   - [PropRendererCreateWindowPointer]: the window where rendering is
//     displayed, required if this isn't a software renderer using a surface
//   - [PropRendererCreateSurfacePointer]: the surface where rendering
//     is displayed, if you want a software renderer without a window
//   - [PropRendererCreateOutputColorspaceNumber]: a [Colorspace]
//     value describing the colorspace for output to the display, defaults to
//     [ColorspaceSRGB]. The direct3d11, direct3d12, and metal renderers
//     support [ColorspaceSRGBLinear], which is a linear color space and
//     supports HDR output. If you select [ColorspaceSRGBLinear], drawing
//     still uses the sRGB colorspace, but values can go beyond 1.0 and float
//     (linear) format textures can be used for HDR content.
//   - [PropRendererCreatePresentVsyncNumber]: non-zero if you want
//     present synchronized with the refresh rate. This property can take any
//     value that is supported by [Renderer.SetVSync] for the renderer.
//
// With the vulkan renderer:
//
//   - [PropRendererCreateVulkanInstancePointer]: the VkInstance to use
//     with the renderer, optional.
//   - [PropRendererCreateVulkanSurfaceNumber]: the VkSurfaceKHR to use
//     with the renderer, optional.
//   - [PropRendererCreateVulkanPhysicalDevicePointer]: the
//     VkPhysicalDevice to use with the renderer, optional.
//   - [PropRendererCreateVulkanDevicePointer]: the VkDevice to use
//     with the renderer, optional.
//   - [PropRendererCreateVulkanGraphicsQueueFamilyIndexNumber]: the
//     queue family index used for rendering.
//   - [PropRendererCreateVulkanPresentQueueFamilyIndexNumber]: the
...
// create a software renderer, but they are intended to be used with a
// [Window] as the final destination and not a [Surface].
//
// surface: the [Surface] structure representing the surface where
// rendering is done.
//
// Returns a valid rendering context or an error.
//
...
//
// Returns the rendering context or an error.
//
// It is safe to call this function from any thread.
//
// This function is available since SDL 3.2.0.
...
//
// Returns the window or an error.
//
// It is safe to call this function from any thread.
//
// This function is available since SDL 3.2.0.
...
//
// Returns the name of the selected renderer or an error.
//
// It is safe to call this function from any thread.
//
// This function is available since SDL 3.2.0.
...
//
//   - [PropRendererNameString]: the name of the rendering driver
//   - [PropRendererWindowPointer]: the window where rendering is
//     displayed, if any
//   - [PropRendererSurfacePointer]: the surface where rendering is
//     displayed, if this is a software renderer without a window
//   - [PropRendererVSyncNumber]: the current vsync setting
//   - [PropRendererMaxTextureSizeNumber]: the maximum texture width
//     and height
//   - [PropRendererTextureFormatsPointer]: a (const SDL_PixelFormat *)
//     array of pixel formats, terminated with [PixelformatUnknown],
//     representing the available texture formats for this renderer.
//   - PropRendererOutputColorspaceNumber: a [Colorspace] value
//     describing the colorspace for output to the display, defaults to
//     [ColorspaceSRGB].
//   - [PropRendererHDREnabledBoolean]: true if the output colorspace is
//     [ColorspaceSRGBLinear] and the renderer is showing on a display with
//     HDR enabled. This property can change dynamically when
//     [EventWindowHDRStateChanged] is sent.
//   - [PropRendererSDRWhitePointFloat]: the value of SDR white in the
//     [ColorspaceSRGBLinear] colorspace. When HDR is enabled, this value is
//     automatically multiplied into the color scale. This property can change
//     dynamically when [EventWindowHDRStateChanged] is sent.
//   - [PropRendererHDRHeadroomFloat]: the additional high dynamic range
...
//
//   - [PropRendererD3D9DevicePointer]: the IDirect3DDevice9 associated
//     with the renderer
//
// With the direct3d11 renderer:
//
//   - [PropRendererD3D11DevicePointer]: the ID3D11Device associated
//     with the renderer
//   - [PropRendererD3D11SwapchainPointer]: the IDXGISwapChain1
//     associated with the renderer. This may change when the window is resized.
//
// With the direct3d12 renderer:
//
//   - [PropRendererD3D12DevicePointer]: the ID3D12Device associated
//     with the renderer
//   - [PropRendererD3D12SwapchainPointer]: the IDXGISwapChain4
//     associated with the renderer.
//   - [PropRendererD3D12CommandQueuePointer]: the ID3D12CommandQueue
//     associated with the renderer
//
// With the vulkan renderer:
//
//   - [PropRendererVulkanInstancePointer]: the VkInstance associated
//     with the renderer
//   - [PropRendererVulkanSurfaceNumber]: the VkSurfaceKHR associated
//     with the renderer
//   - [PropRendererVulkanPhysicalDevicePointer]: the VkPhysicalDevice
//     associated with the renderer
//   - [PropRendererVulkanDevicePointer]: the VkDevice associated with
//     the renderer
//   - [PropRendererVulkanGraphicsQueueFamilyIndexNumber]: the queue
//     family index used for rendering
//   - [PropRendererVulkanPresentQueueFamilyIndexNumber]: the queue
//     family index used for presentation
//   - [PropRendererVulkanSwapchainImageCountNumber]: the number of
//     swapchain images, or potential frames in flight, used by the Vulkan
//     renderer
//
// With the gpu renderer:
//
//   - [PropRendererGPUDevicePointer]: the [GPUDevice] associated with
//     the renderer
//
// renderer: the rendering context.
//
...
//
// Returns the output size in pixels or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
// logical size, otherwise it will return the value of [Renderer.OutputSize].
//
// renderer: the rendering context.
//
// w: the current width.
...
//
// Returns the current output size in pixels or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
//
// Returns the created texture or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
//
// The [TextureAccess] hint for the created texture is [TextureaccessStatic].
//
// The pixel format of the created texture may be different from the pixel
// format of the surface, and can be queried using the
// [PropTextureFormatNumber] property.
//
// renderer: the rendering context.
//
// surface: the [Surface] structure containing pixel data used to fill
// the texture.
//
// Returns the created texture or an error.
//
...
// These are the supported properties:
//
//   - [PropTextureCreateColorspaceNumber]: a [Colorspace] value
//     describing the texture colorspace, defaults to [ColorspaceSRGBLinear]
//     for floating point textures, [ColorspaceHDR10] for 10-bit textures,
//     [ColorspaceSRGB] for other RGB textures and [ColorspaceJPEG] for
//     YUV textures.
//   - [PropTextureCreateFormatNumber]: one of the enumerated values in
//     [PixelFormat], defaults to the best RGBA format for the renderer
//   - [PropTextureCreateAccessNumber]: one of the enumerated values in
//     [TextureAccess], defaults to [TextureaccessStatic]
//   - [PropTextureCreateWidthNumber]: the width of the texture in
//     pixels, required
//   - [PropTextureCreateHeightNumber]: the height of the texture in
//     pixels, required
//   - [PropTextureCreateSDRWhitePointFloat]: for HDR10 and floating
//     point textures, this defines the value of 100% diffuse white, with higher
//     values being displayed in the High Dynamic Range headroom. This defaults
//     to 100 for HDR10 textures and 1.0 for floating point textures.
//   - [PropTextureCreateHDRHeadroomFloat]: for HDR10 and floating
//     point textures, this defines the maximum dynamic range used by the
//     content, in terms of the SDR white point. This would be equivalent to
//     maxCLL / [PropTextureCreateSDRWhitePointFloat] for HDR10 content.
...
// With the direct3d11 renderer:
//
//   - [PropTextureCreateD3D11TexturePointer]: the ID3D11Texture2D
//     associated with the texture, if you want to wrap an existing texture.
//   - [PropTextureCreateD3D11TextureUPointer]: the ID3D11Texture2D
//     associated with the U plane of a YUV texture, if you want to wrap an
//     existing texture.
//   - [PropTextureCreateD3D11TextureVPointer]: the ID3D11Texture2D
//     associated with the V plane of a YUV texture, if you want to wrap an
//     existing texture.
//
// With the direct3d12 renderer:
//
//   - [PropTextureCreateD3D12TexturePointer]: the ID3D12Resource
//     associated with the texture, if you want to wrap an existing texture.
//   - [PropTextureCreateD3D12TextureUPointer]: the ID3D12Resource
//     associated with the U plane of a YUV texture, if you want to wrap an
//     existing texture.
//   - [PropTextureCreateD3D12TextureVPointer]: the ID3D12Resource
//     associated with the V plane of a YUV texture, if you want to wrap an
//     existing texture.
//
// With the metal renderer:
//
//   - [PropTextureCreateMetalPixelbufferPointer]: the CVPixelBufferRef
//     associated with the texture, if you want to create a texture from an
//     existing pixel buffer.
//
// With the opengl renderer:
//
//   - [PropTextureCreateOpenGLTextureNumber]: the GLuint texture
//     associated with the texture, if you want to wrap an existing texture.
//   - [PropTextureCreateOpenGLTextureUVNumber]: the GLuint texture
//     associated with the UV plane of an NV12 texture, if you want to wrap an
//     existing texture.
//   - [PropTextureCreateOpenGLTextureUNumber]: the GLuint texture
//     associated with the U plane of a YUV texture, if you want to wrap an
//     existing texture.
//   - [PropTextureCreateOpenGLTextureVNumber]: the GLuint texture
//     associated with the V plane of a YUV texture, if you want to wrap an
//     existing texture.
//
// With the opengles2 renderer:
//
//   - [PropTextureCreateOpenGLES2TextureNumber]: the GLuint texture
//     associated with the texture, if you want to wrap an existing texture.
//   - [PropTextureCreateOpenGLES2TextureUVNumber]: the GLuint texture
//     associated with the UV plane of an NV12 texture, if you want to wrap an
//     existing texture.
//   - [PropTextureCreateOpenGLES2TextureUNumber]: the GLuint texture
//     associated with the U plane of a YUV texture, if you want to wrap an
//     existing texture.
//   - [PropTextureCreateOpenGLES2TextureVNumber]: the GLuint texture
//     associated with the V plane of a YUV texture, if you want to wrap an
//     existing texture.
//
// With the vulkan renderer:
//
//   - [PropTextureCreateVulkanTextureNumber]: the VkImage with layout
...
// The following read-only properties are provided by SDL:
//
//   - [PropTextureColorspaceNumber]: a [Colorspace] value describing
//     the texture colorspace.
//   - [PropTextureFormatNumber]: one of the enumerated values in
//     [PixelFormat].
//   - [PropTextureAccessNumber]: one of the enumerated values in
//     [TextureAccess].
//   - [PropTextureWidthNumber]: the width of the texture in pixels.
//   - [PropTextureHeightNumber]: the height of the texture in pixels.
//   - [PropTextureSDRWhitePointFloat]: for HDR10 and floating point
//     textures, this defines the value of 100% diffuse white, with higher
//     values being displayed in the High Dynamic Range headroom. This defaults
//     to 100 for HDR10 textures and 1.0 for other textures.
//   - [PropTextureHDRHeadroomFloat]: for HDR10 and floating point
...
// With the direct3d11 renderer:
//
//   - [PropTextureD3D11TexturePointer]: the ID3D11Texture2D associated
//     with the texture
//   - [PropTextureD3D11TextureUPointer]: the ID3D11Texture2D
//     associated with the U plane of a YUV texture
//   - [PropTextureD3D11TextureVPointer]: the ID3D11Texture2D
//     associated with the V plane of a YUV texture
//
// With the direct3d12 renderer:
//
//   - [PropTextureD3D12TexturePointer]: the ID3D12Resource associated
//     with the texture
//   - [PropTextureD3D12TextureUPointer]: the ID3D12Resource associated
//     with the U plane of a YUV texture
//   - [PropTextureD3D12TextureVPointer]: the ID3D12Resource associated
//     with the V plane of a YUV texture
//
// With the vulkan renderer:
//
//   - [PropTextureVulkanTextureNumber]: the VkImage associated with the
//     texture
//
// With the opengl renderer:
//
//   - [PropTextureOpenGLTextureNumber]: the GLuint texture associated
//     with the texture
//   - [PropTextureOpenGLTextureUVNumber]: the GLuint texture
//     associated with the UV plane of an NV12 texture
//   - [PropTextureOpenGLTextureUNumber]: the GLuint texture associated
//     with the U plane of a YUV texture
//   - [PropTextureOpenGLTextureVNumber]: the GLuint texture associated
//     with the V plane of a YUV texture
//   - [PropTextureOpenGLTextureTargetNumber]: the GLenum for the
//     texture target (GL_TEXTURE_2D, GL_TEXTURE_RECTANGLE_ARB, etc)
//   - [PropTextureOpenGLTexWFloat]: the texture coordinate width of
//     the texture (0.0 - 1.0)
//   - [PropTextureOpenGLTexHFloat]: the texture coordinate height of
//     the texture (0.0 - 1.0)
//
// With the opengles2 renderer:
//
//   - [PropTextureOpenGLES2TextureNumber]: the GLuint texture
//     associated with the texture
//   - [PropTextureOpenGLES2TextureUVNumber]: the GLuint texture
//     associated with the UV plane of an NV12 texture
//   - [PropTextureOpenGLES2TextureUNumber]: the GLuint texture
//     associated with the U plane of a YUV texture
//   - [PropTextureOpenGLES2TextureVNumber]: the GLuint texture
//     associated with the V plane of a YUV texture
//   - [PropTextureOpenGLES2TextureTargetNumber]: the GLenum for the
...
// texture: the texture to query.
//
// Returns a pointer to the [Renderer] that created the texture, or an error.
//
// It is safe to call this function from any thread.
//
// This function is available since SDL 3.2.0.
...
//
// Returns the color modulation values or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
// Set the blend mode for a texture, used by [Renderer.Texture].
//
...
//
// The default texture scale mode is [ScalemodeLinear].
//
...
// texture: the texture to query.
//
// Returns the current scale mode or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
// queried using the [PropTextureFormatNumber] property.
//
...
// texture: the texture to update.
//
// rect: a [Rect] structure representing the area to update, or nil
...
//
// rect: a [Rect] structure representing the area to lock for access;
// nil to lock the entire texture.
//
// pixels: a pointer to the locked pixels,
// appropriately offset by the locked area.
//
// pitch: the pitch of the locked pixels; the
// pitch is the length of one row in bytes.
//
// Returns an error if the texture is not valid or was not
// created with [TextureaccessStreaming].
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
//
// Returns the surface or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
// [TextureaccessTarget] flag, or nil to render to the
...
// [LogicalPresentationDisabled], and in that case you get the full pixel
...
// [Renderer.Present]; be sure to reenable it first if you were using it.
//
...
// mode: the presentation mode used.
//
// Returns resolution and presentation mode or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
// renderer: the rendering context.
//
// rect: the final presentation rectangle.
//
// Returns the final presentation rectangle or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
// renderer: the rendering context.
//
// windowX: the x coordinate in window coordinates.
//
// windowY: the y coordinate in window coordinates.
//
// x: the x coordinate in render coordinates.
//
// y: the y coordinate in render coordinates.
//
// Returns the coordinates or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
// y: the y coordinate in render coordinates.
//
// windowX: the x coordinate in window
// coordinates.
//
// windowY: the y coordinate in window
// coordinates.
//
// Returns the coordinates or an error.
//
...
// [Renderer.CoordinatesFromWindow] on the specific event fields instead of
...
// renderer: the rendering context.
//
// rect: the [Rect] structure representing the drawing area, or nil
...
// renderer: the rendering context.
//
// rect: the current drawing area.
//
// Returns the drawing area or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
// renderer: the rendering context.
//
// rect: the area that is safe for interactive
// content.
//
// Returns nil on success or an error on failure.
//
...
// renderer: the rendering context.
//
// rect: a [Rect] structure representing the clip area, relative to
...
// scaleY: the vertical scaling factor.
//
// Returns the drawing scale or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
// [AlphaOpaque] (255). Use [Renderer.SetDrawBlendMode] to
...
// renderer: the rendering context.
//
// r: the red value used to draw on the
// rendering target.
//
// g: the green value used to draw on the
// rendering target.
//
// b: the blue value used to draw on the
// rendering target.
//
// a: the alpha value used to draw on the
// rendering target; usually [AlphaOpaque] (255).
//
// Returns the color or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
// renderer: the rendering context.
//
// scale: the current color scale value.
//
// Returns the color scale or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
//
// points: the points along the lines, drawing len(points)-1 lines.
//
// Returns nil on success or an error on failure.
//
// This function should only be called on the main thread.
...
//
// rect: a pointer to the destination rectangle, or nil to outline the
...
//
// rects: a slice of destination rectangles.
//
// Returns nil on success or an error on failure.
//
// This function should only be called on the main thread.
...
//
// srcrect: a pointer to the source rectangle, or nil for the entire
// texture.
//
...
//
// flip: an [FlipMode] value stating which flipping actions should be
...
// corner of dstrect, or 0 for an unscaled copy.
//
// dstrect: a pointer to the destination rectangle, or nil for the
// entire rendering target.
//
...
func (renderer *Renderer) Geometry(texture *Texture, vertices []Vertex, indices []int32) error {
	...
	if texture != nil {
		ctexture = texture.internal
	}
	if !C.SDL_RenderGeometry((*C.SDL_Renderer)(renderer), ctexture, (*C.SDL_Vertex)(unsafe.Pointer(unsafe.SliceData(vertices))), (C.int)(len(vertices)), (*C.int)(unsafe.Pointer(unsafe.SliceData(indices))), (C.int)(len(indices))) {
		return getError()
	}
	return nil
}
...
func (renderer *Renderer) GeometryRaw(texture *Texture, xy *float32, xy_stride int32, color *FColor, color_stride int32, uv *float32, uv_stride int32, num_vertices int32, indices unsafe.Pointer, num_indices int32, size_indices int32) error {
	...
	if !C.SDL_RenderGeometryRaw((*C.SDL_Renderer)(renderer), ctexture, (*C.float)(xy), (C.int)(xy_stride), (*C.SDL_FColor)(unsafe.Pointer(color)), (C.int)(color_stride), (*C.float)(uv), (C.int)(uv_stride), (C.int)(num_vertices), indices, (C.int)(num_indices), (C.int)(size_indices)) {
		return getError()
	}
	return nil
}
...
// rendering function such as [Renderer.Line] does not directly put a line on
...
// encouraged to call [Renderer.Clear] to initialize the backbuffer before
...
// target to default via [Renderer.SetRenderTarget](nil) afterwards, as
...
//
// Passing an invalid texture will set the SDL error message
// to "Invalid texture".
//
...
// [PropRendererVulkanSwapchainImageCountNumber] will give you the
// maximum number of semaphores you'll need.
//
// renderer: the rendering context.
//
// waitStageMask: the VkPipelineStageFlags for the wait.
//
// waitSemaphore: a VkSempahore to wait on before rendering the current
// frame, or 0 if not needed.
//
// signalSemaphore: a VkSempahore that SDL will signal when rendering
...
func (renderer *Renderer) AddVulkanSemaphores(waitStageMask uint32, waitSemaphore int64, signalSemaphore int64) error {
	if !C.SDL_AddVulkanRenderSemaphores((*C.SDL_Renderer)(renderer), (C.Uint32)(waitStageMask), (C.Sint64)(waitSemaphore), (C.Sint64)(signalSemaphore)) {
		return getError()
	}
	return nil
}
...
//
// When a renderer is created, vsync defaults to [RendererVSyncDisabled].
//
// The vsync parameter can be 1 to synchronize present with every vertical
// refresh, 2 to synchronize present with every second vertical refresh, etc.,
// [RendererVSyncAdaptive] for late swap tearing (adaptive vsync), or
// [RendererVSyncDisabled] to disable. Not every value is supported by
...
//
// Returns the vsync setting or an error.
//
// This function should only be called on the main thread.
//
// This function is available since SDL 3.2.0.
...
// The size, in pixels, of a single [Renderer.DebugText] character.
//
...
//
// This function will render a string of text to a [Renderer]. Note that
